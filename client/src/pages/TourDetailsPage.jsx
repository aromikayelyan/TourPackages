...
import { useEffect, useState } from 'react'
import { useParams, useNavigate } from 'react-router-dom'
import { createRating, fetchTourByUid } from '../api.js' // addToCart и createChat остаются без изменений
import { addToCart, createChat } from '../api.js' 
import { useAuth } from '../AuthContext.jsx'
...
function TourDetailsPage() {
  ...
  // Функция для загрузки отзывов из localStorage локально
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (tourData) {
          // Важно: Превращаем строку картинок "url1,url2" в массив для карусели
          if (tourData.images && typeof tourData.images === 'string') {
            tourData.images = tourData.images.split(',')
          } else if (!tourData.images) {
            tourData.images = []
          }

          setTour(tourData)
          
          // Загружаем отзывы для этого тура
          const commentsData = loadLocalComments()
          setComments(commentsData)
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createRating(uid, {
        id: Date.now(), // Добавляем фейковый id для рендера списков key={}
        userName: form.userName,
        comment: form.comment,
        rate: Number(form.rate),
      })

      // Обновляем список отзывов сразу из локального хранилища
      const updatedComments = loadLocalComments()
  ...
  const handleAddToCart = async () => {
    ...
    // Проверяем как user.userUId так и user.uid на случай разных названий полей в AuthContext
    const currentUserId = user.userUId || user.uid
    if (!currentUserId) {
      alert('User ID is missing. Please sign in again.')
      return
    }
    ...
    setCartMessage('')
    try {
      await addToCart(currentUserId, uid)
      setCartMessage('✓ Added to cart successfully!')
      setTimeout(() => setCartMessage(''), 3000)
    } catch (err) {
      console.error('Add to cart error:', err)
  ...
  // Безопасная проверка для отрисовки кнопки чата
  const currentUserId = user?.userUId || user?.uid
  ...
  return (
    <div className="page">
        ...
        <div className="tour-info">
          ...
          <div className="tour-info-card">
            ...
            <div className="tour-actions">
              ...
              {user && (
                <>
                  {tour.creatorUserUId && tour.creatorUserUId !== currentUserId && (
                    <button
                      className="btn btn-chat-organizer"
                      onClick={() => handleStartChat(tour.creatorUserUId)}
                      disabled={creatingChat}
      ...
      <section className="section">
        ...
        <ul className="reviews">
          {comments.map((c) => (
            <li key={c.id || c.created_at} className="review">
              <div className="review-header">
                <strong>{c.userName}</strong>
                <span>Rate: {c.rate}/5</span>
              </div>
